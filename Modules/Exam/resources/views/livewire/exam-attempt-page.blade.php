...
<div>
    @if ($attempt->started_at==null)
        <div class="min-h-[70vh] flex items-center justify-center px-4">
            <div class="w-full max-w-lg rounded-2xl bg-white shadow-xl border border-gray-200 p-8">

                <div class="flex justify-center mb-4">
                    <div class="flex h-16 w-16 items-center justify-center rounded-full bg-yellow-100">
                        <x-icon name="exclamation-triangle" class="w-8 h-8 text-yellow-600" />
                    </div>
                </div>
                ...
                <p class="mt-4 text-center text-gray-600">
                    Sau khi bắt đầu, thời gian làm bài sẽ được tính ngay và
                    <span class="font-semibold text-red-600">không thể tạm dừng</span>.
                </p>
                ...
                <div class="mt-8 flex justify-center gap-3">
                    {{-- <x-button
                        flat
                        label="Quay lại"
                        href="{{ route('exams.detail', $exam->slug) }}"
                    /> --}}

                    <x-button
                        positive
                        label="Bắt đầu bài thi"
    ...
    @else
        <div 
            ...
            {{-- Banner đăng nhập cho guest user với Popup --}}
            @guest
            <div class="bg-gradient-to-r from-indigo-500 to-purple-600 border-b border-indigo-700"
                x-data="{
                    openLoginPopup() {
                        ...
                        const popup = window.open(
                            '{{ route('auth.google.redirect', ['popup' => '1', 'exam_session' => $attempt->session_id]) }}',
                            'GoogleLogin',
                            `width=${width},height=${height},left=${left},top=${top},popup=1,scrollbars=yes`
                        );
                        
                        // Lắng nghe message từ popup sau khi đăng nhập thành công
                        const messageHandler = (event) => {
                            // Kiểm tra message từ popup
                ...
                <div class="max-w-7xl mx-auto px-4 py-3">
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-3">
                        <div class="flex items-center gap-3 text-white">
                            ...
                            <div>
                                <p class="font-semibold text-sm sm:text-base">Bạn chưa đăng nhập</p>
                                <p class="text-xs sm:text-sm text-indigo-100">Đăng nhập để lưu kết quả bài thi vào tài khoản của bạn</p>
                            </div>
                        </div>
                        <button 
                            @click="openLoginPopup()"
                            type="button"
                            class="flex items-center gap-2 bg-white text-indigo-600 px-4 py-2 rounded-lg font-semibold text-sm hover:bg-indigo-50 transition-colors whitespace-nowrap shadow-lg">
