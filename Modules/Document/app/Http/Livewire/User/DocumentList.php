...
class DocumentList extends Component
{
    ...
    public function updated($propertyName)
    {
        if (in_array($propertyName, [
            'search', 'selectedCategory', 'selectedFileType', 'selectedPrice', 
    ...
    public function render()
    {
        ...
        if (!empty($this->search)) {
            $query->where(function($q) {
                $q->where('documents.title', 'like', '%' . $this->search . '%')
                  ->orWhere('documents.short_description', 'like', '%' . $this->search . '%')
                  ->orWhere('documents.description', 'like', '%' . $this->search . '%');
            });
        }

        if ($this->selectedCategory) {
            $query->where('documents.category_id', $this->selectedCategory);
        }

        if (!empty($this->selectedFileType)) {
            $query->where('documents.file_type', $this->selectedFileType);
        }
        ...
        if (!empty($this->selectedResourceType)) {
            if ($this->selectedResourceType === 'pdf') {
                ...
                $query->where(function($q) {
                    $q->whereIn('file_type', ['zip', 'rar', 'tar', 'gz', '7z'])
        ...
        if (!empty($this->selectedSubject)) {
            if ($this->selectedSubject === 'lap_trinh_web') {
                $query->where(function($q) {
                    $q->where('title', 'like', '%lập trình web%')
                ...
                $query->where(function($q) {
                    $q->where('title', 'like', '%mạng máy tính%')
        ...
        if (!empty($this->selectedYear)) {
            $query->whereYear('documents.published_at', $this->selectedYear);
        }
        ...
        if ($this->sort === 'newest') {
            $query->orderBy('documents.published_at', 'desc');
        } elseif ($this->sort === 'popular') {
            $query->orderBy('documents.download_count', 'desc');
            ...
            $query->orderByRaw('COALESCE((SELECT AVG(dr.rating) FROM document_reviews dr WHERE dr.document_id = documents.id), 0) DESC')
                ->orderBy('documents.download_count', 'desc');
        }

        $documents = $query->with(['author', 'category', 'product', 'tags', 'favorites' => function($q) {
