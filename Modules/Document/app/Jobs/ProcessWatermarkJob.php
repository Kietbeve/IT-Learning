    ...
    public function handle()
    {
        ...
        try {
            ...
            if (in_array($fileType, ['pdf', 'docx'])) {
                ...
                // Generate preview (NO watermark - use original file for consistency)
                if ($fileType === 'pdf') {
                    // PDF: Generate preview from ORIGINAL PDF (no watermark)
                    try {
                        // Use local original file directly (no watermark)
                        $previewR2Path = $watermarkService->generatePreview($originalPath, 'pdf');

                        if ($previewR2Path) {
                            $pendingVersion->update(['preview_file_path' => $previewR2Path]);
                            Log::info('Preview generated from original', ['doc_id' => $document->id, 'preview_path' => $previewR2Path]);
                        } else {
                            // Fallback: use original file as preview
                            $pendingVersion->update(['preview_file_path' => $pendingVersion->file_original_path]);
                            Log::info('Preview fallback to original', ['doc_id' => $document->id]);
                        }
                    ...
                    // DOCX: Convert ORIGINAL DOCX to PDF for preview (no watermark)
                    try {
                        // Convert original DOCX to PDF (no watermark)
                        $tempPdf = $watermarkService->convertDocxToPdfForPreview($originalPath);
                        ...
                        if ($tempPdf && file_exists($tempPdf)) {
                            ...
                            if ($previewR2Path) {
                                $pendingVersion->update(['preview_file_path' => $previewR2Path]);
                                Log::info('DOCX preview generated from original', ['doc_id' => $document->id, 'preview_path' => $previewR2Path]);
                            }
