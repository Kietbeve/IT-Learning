...
class DocumentDatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Lấy hoặc tạo môn học mẫu
        $sub1 = \Modules\Document\Models\Subject::firstOrCreate(
            ['slug' => 'lap-trinh-web'],
            ['name' => 'Lập trình Web', 'category_id' => $catLaravel->id, 'is_active' => true]
        );
        $sub2 = \Modules\Document\Models\Subject::firstOrCreate(
            ['slug' => 'php-co-ban'],
            ['name' => 'PHP Cơ Bản', 'category_id' => $catPHP->id, 'is_active' => true]
        );

        // 4. Sinh dữ liệu mẫu Tài liệu (Documents) + Versions
        ...
        $ver1 = DocumentVersion::updateOrCreate(
            ...
            [
                ...
                'status' => 'approved',
                'submitted_by' => $contributorUser->id,
                'submitted_at' => now(),
        ...
        $ver2 = DocumentVersion::updateOrCreate(
            ['document_id' => $doc2->id, 'version_number' => 1],
            [
                ...
                'file_watermarked_path' => 'documents/source-code-php-web.zip',
                'watermark_status' => 'success',
                'file_type' => 'zip',
        ...
        $ver3 = DocumentVersion::updateOrCreate(
            ['document_id' => $doc3->id, 'version_number' => 1],
            [
        ...
        DocumentVersion::updateOrCreate(
            ['document_id' => $doc4->id, 'version_number' => 1],
            [
        ...
        $ver5 = DocumentVersion::updateOrCreate(
            ['document_id' => $doc5->id, 'version_number' => 1],
            [
        ...
        $doc5->update(['current_version_id' => $ver5->id]);
        ...
        for ($i = 1; $i <= 25; $i++) {
            $docLoop = Document::updateOrCreate(
            ...
            $verLoop = DocumentVersion::updateOrCreate(
                ['document_id' => $docLoop->id, 'version_number' => 1],
                [
                    ...
                    'subject_id' => ($i % 2 === 0) ? $sub1->id : $sub2->id,
                    ...
                    'status' => 'approved',
                    'submitted_by' => $contributorUser->id,
                    'submitted_at' => now()->subYears($i % 3)->subDays($i),
                    'reviewed_by' => $adminUser->id,
                    'reviewed_at' => now()->subYears($i % 3)->subDays($i),
                    'price' => 0,
                ]
            );
            $docLoop->update(['current_version_id' => $verLoop->id]);
        }
    }
}
