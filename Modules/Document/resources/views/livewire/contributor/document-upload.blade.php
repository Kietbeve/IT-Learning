<div class="max-w-4xl mx-auto space-y-6 font-sans pb-10">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-extrabold text-slate-850 tracking-tight">Tải Lên Tài Liệu</h2>
            <p class="text-xs text-slate-400 mt-1">Chia sẻ tài nguyên hữu ích và tích lũy doanh thu của bạn.</p>
        </div>
        ...
        <div class="md:col-span-2 space-y-6">
            ...
            <div class="rounded-[2rem] border border-white bg-white/70 p-6 shadow-xl shadow-slate-100/50 backdrop-blur-md space-y-5">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="title" wire:model.blur="title" placeholder="VD: Giáo trình cấu trúc dữ liệu và giải thuật" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:border-indigo-400 focus:outline-none transition-colors" />
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea id="description" wire:model.blur="description" rows="6" placeholder="Mô tả cụ thể tài liệu gồm những phần nào, kiến thức gì, lợi ích khi học viên đọc tài liệu này..." class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:border-indigo-400 focus:outline-none transition-colors"></textarea>
            ...
            <!-- Upload File Card -->
            <div class="rounded-[2rem] border border-white bg-white/70 p-6 shadow-xl shadow-slate-100/50 backdrop-blur-md space-y-5">
                <h3 class="text-sm font-extrabold text-slate-800 border-b border-slate-100 pb-3 uppercase tracking-wider">Tệp đính kèm</h3>

                <!-- File Dropzone -->
                <div class="space-y-3">
                    <label class="text-xs font-extrabold text-slate-700">Chọn tệp tài liệu <span class="text-rose-500">*</span></label>
                    <div class="relative group border-2 border-dashed border-slate-200 rounded-3xl p-6 text-center hover:border-indigo-400 transition-colors bg-slate-50/50">
                        <input type="file" id="originalFile" wire:model="originalFile" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept=".pdf,.docx,.zip" />
                        ...
                        <div class="space-y-2">
                            ...
                            <p class="text-[10px] text-slate-400 font-bold">PDF, DOCX, ZIP tối đa 50MB</p>
                        </div>
                    </div>
                    ...
                    @if ($originalFile)
                        <div class="flex items-center justify-between bg-indigo-50/40 border border-indigo-100 rounded-2xl p-3 text-xs text-slate-700">
                            <div class="flex items-center gap-2 overflow-hidden">
                                ...
                                <div class="min-w-0">
                                    <p class="font-bold text-slate-900 truncate" title="{{ $originalFile->getClientOriginalName() }}">{{ $originalFile->getClientOriginalName() }}</p>
                                    <p class="text-[9px] text-slate-450 font-bold uppercase mt-0.5">{{ number_format($originalFile->getSize() / 1024 / 1024, 2) }} MB</p>
                                </div>
                            </div>
                            <button type="button" wire:click="$set('originalFile', null)" class="text-rose-600 hover:text-rose-700 font-extrabold px-3 py-1.5 uppercase text-[9px] tracking-wider bg-rose-50 hover:bg-rose-100 rounded-xl transition-colors shrink-0">Hủy</button>
                        </div>
                    @endif
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-[2rem] border border-white bg-white/70 p-6 shadow-xl shadow-slate-100/50 backdrop-blur-md space-y-5">
                ...
                <div class="space-y-3" x-data="{ paid: @entangle('isPaid') }">
                    <label class="text-xs font-extrabold text-slate-700 block">Hình thức xuất bản</label>
                ...
                <div class="flex items-center justify-between pt-2">
                    <div class="space-y-0.5">
                        ...
                        <span class="text-[10px] text-slate-450 font-bold block">Cho phép học viên lưu trữ file</span>
                    </div>
            ...
            <div class="rounded-[2rem] border border-white bg-white/70 p-6 shadow-xl shadow-slate-100/50 backdrop-blur-md space-y-5">
                ...
                <div class="space-y-3">
                    ...
                    @if ($thumbnailFile)
                        <div class="relative rounded-2xl overflow-hidden border border-slate-250">
                            <img src="{{ $thumbnailFile->temporaryUrl() }}" class="w-full h-32 object-cover" alt="Cover preview" />
                            <button type="button" wire:click="$set('thumbnailFile', null)" class="absolute top-2 right-2 rounded-full bg-rose-600 text-white p-1 hover:bg-rose-700 shadow-md transition-colors" title="Xóa ảnh bìa">
