<div class="max-w-4xl mx-auto space-y-6 font-sans pb-10">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-extrabold text-gray-850 tracking-tight">Chỉnh Sửa Tài Liệu</h2>
            <p class="text-xs text-gray-400 mt-1">Cập nhật thông tin chi tiết hoặc thay thế tệp tin tài liệu của bạn.</p>
        </div>
        ...
        <div class="md:col-span-2 space-y-6">
            ...
            <div class="rounded-[2rem] border border-white bg-white/70 p-6 shadow-xl shadow-gray-100/50 backdrop-blur-md space-y-5">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="title" wire:model.blur="title" placeholder="VD: Giáo trình cấu trúc dữ liệu và giải thuật" class="w-full rounded-2xl border border-gray-200 px-4 py-3 text-xs font-semibold text-gray-800 placeholder-gray-400 focus:border-indigo-500 focus:outline-none transition-colors" />
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea id="description" wire:model.blur="description" rows="6" placeholder="Mô tả cụ thể tài liệu gồm những phần nào, kiến thức gì..." class="w-full rounded-2xl border border-gray-200 px-4 py-3 text-xs font-semibold text-gray-800 placeholder-gray-400 focus:border-indigo-500 focus:outline-none transition-colors"></textarea>
            ...
            <!-- Upload File Card -->
            <div class="rounded-[2rem] border border-white bg-white/70 p-6 shadow-xl shadow-gray-100/50 backdrop-blur-md space-y-5">
                <h3 class="text-sm font-extrabold text-gray-800 border-b border-gray-100 pb-3 uppercase tracking-wider">Tệp tài liệu đính kèm</h3>

                <!-- Existing File Info -->
                <div class="space-y-1.5">
                    <span class="text-xs font-extrabold text-gray-500 uppercase">Tệp hiện tại</span>
                    <div class="flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-2xl p-4 text-xs text-gray-700">
                        <svg class="w-8 h-8 text-gray-450 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                        <div class="min-w-0 flex-1">
                            <span class="font-bold text-gray-900 block truncate text-sm" title="{{ $doc->title }}">{{ $doc->title }}</span>
                            <span class="block text-[10px] text-gray-400 font-bold uppercase mt-0.5">{{ strtoupper($doc->file_type) }} • Lưu trữ trên cloud</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    <label class="text-xs font-extrabold text-gray-350">Thay thế bằng tệp mới (tùy chọn)</label>
                    <div class="relative group border-2 border-dashed border-gray-200 rounded-3xl p-6 text-center hover:border-indigo-400 transition-colors bg-gray-50/50">
                        ...
                        <div class="space-y-2">
                            ...
                            <p class="text-[10px] text-gray-450 font-bold">PDF, DOC, DOCX, ZIP tối đa 50MB</p>
                        </div>
                    </div>
                    ...
                    @if ($newFile)
                        <div class="flex items-center justify-between bg-indigo-50/40 border border-indigo-100 rounded-2xl p-3 text-xs text-gray-700">
                            <div class="flex items-center gap-2 overflow-hidden">
                                ...
                                <div class="min-w-0">
                                    <p class="font-bold text-gray-900 truncate" title="{{ $newFile->getClientOriginalName() }}">{{ $newFile->getClientOriginalName() }}</p>
                                    <p class="text-[9px] text-gray-450 font-bold uppercase mt-0.5">{{ number_format($newFile->getSize() / 1024 / 1024, 2) }} MB</p>
                                </div>
                            </div>
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-[2rem] border border-white bg-white/70 p-6 shadow-xl shadow-gray-100/50 backdrop-blur-md space-y-5">
                ...
                <div class="space-y-3" x-data="{ paid: @entangle('isPaid') }">
                    <label class="text-xs font-extrabold text-gray-350 block">Hình thức xuất bản</label>
                ...
                <div class="flex items-center justify-between pt-2">
                    <div class="space-y-0.5">
                        ...
                        <span class="text-[10px] text-gray-450 font-bold block">Cho phép học viên download file</span>
                    </div>
            ...
            <div class="rounded-[2rem] border border-white bg-white/70 p-6 shadow-xl shadow-gray-100/50 backdrop-blur-md space-y-5">
                ...
                <div class="space-y-3">
                    <label class="text-xs font-extrabold text-gray-700">Ảnh bìa</label>
                    ...
                    @if ($newThumbnailFile)
                        <div class="relative rounded-2xl overflow-hidden border border-gray-255">
                    ...
                    @elseif ($doc->gallery_images && is_array($doc->gallery_images) && count($doc->gallery_images) > 0)
                        <div class="grid grid-cols-5 gap-2">
                            @foreach($doc->gallery_images as $index => $galleryImage)
                                <div class="relative rounded-xl overflow-hidden border border-gray-250 aspect-square">
